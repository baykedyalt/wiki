...
Ultramarine Wiki'ye hoşgeldiniz! Bu wiki kullanıcıların Ultramarine Linux'e geçmesine yardım etmek için tasarlanmıştır.

Geliştirici ve Paket Yöneticisi Belgeleri [Fyra Developer](https://developer.fyralabs.com) sayfasına taşındı. Katkıda bulunmak isterseniz, [Katkıda bulunma](/tr/contributing/contributorguide) bölümüne gidin.

Bu sayfada, proje hakkında yazıları okuyabilirsiniz, yani önemli konseptlere ve düşüncemize aşina olmak isterseniz, ilk bunu okuyun!

Ultramarine Linux'ü indirmek isterseniz, [Ultramarine'i edinmek](/tr/installation/getting) bölümüne gidin.

## Genel Düşünce

Ultramarine Linux, her düzeydeki deneyime sahip insanlar için "sadece işe yarayan" bir işletim sistemi oluşturmayı amaçlayan, Korora Linux'ün manevi halefidir .

Bunu yapmak için bir dizi değere bağlı kalıyoruz:

### Ultramarine Eğiticidir.

Fedora'nın ideolojiye bağlı kalmaya çalıştığı yer, (neredeyse) ücretsiz bir sistem sunmaktır, Ultramarine en az dirence sahip modelde çalışır. Bu, Fedora'nın ilk açıldığı anda rahat edemeyeceği bazı özel sürücüler ve codecler göndermemiz gerekse bile bu kullanıcının işini kolaylaştırmaya çalıştığımız anlamına geliyor.

### Ultramarine Yenilikçidir.

Linux alanındaki son yeniliklerin kullanıcı ve geliştirici deneyimini büyük ölçüde iyileştirdiğine inanıyoruz. Flatpak ve xdg-portals gibi teknolojiler, geliştiricilere kullanıcılarımıza daha tutarlı harika deneyimler sunmalarına olanak tanır. Kaynakta yapılan çalışmalar, kullanıcılarımız için deneyimi iyileştirirken, Ultramarine ekosistemine yapılan çalışmalar; vurgu renkleri ve Chromebook desteği gibi, daha geniş Linux ekosistemini iyileştirir. Yani bu işten herkes kârlı.



### Ultramarine Kullanıcı Dostudur.

Daha deneyimli kullanıcılar için birkaç ayarı ayarlamak ve üçüncü taraf depolarından bazı paketler yüklemek basit görünebilirken, Linux'a yeni olan (veya hatta sadece Fedora'ya yeni olan) kişilerin hangi paketlere ihtiyaç duyduklarını ve onları nereden bulacaklarını bulması saatler alabilir. Ultramarine, yeni ve eski kullanıcıların hızlı bir şekilde işe başlamasını sağlar.

## Eklenmesi Planlanan Özellikler


-Kullanıcıları sistemde hızlı bir şekilde tanıtmak için hoş geldiniz uygulaması, yaygın işlemlere hızlı erişim sağlar.
-YaST'a benzer sistem yapılandırma uygulaması.
-Özel kurulum programı - yeni kullanıcılar için karşılaşılması zor bir arayüze sahip olan Anaconda'ya göre daha kolay kullanılabilir

## SSS

### Birleştirme kodu ne yapar?

Birleştirme kodları RPMFusion'u , Terra depo verilerini , ve bizim imzalı anahtarlarımızı indirir, ardından mevcut bir Fedora kurulumunun üstüne Ultramarine Linux'un en temel kurulumunu (Fedora sürüm paketlerini Ultramarine sürümleriyle değiştirerek) indirir.

Komut dosyası, mevcut sisteminize ekstra şişirme yapmadan Ultramarine Linux'un çalışması için gereken en temel bileşenleri yükler.

Birleştirmeden sonra, onları oluşturduğumuzda en son ve en büyük Ultramarine özelliklerine sahip olacaksınız!

### Bu Linux dağıtımı neden var? Başka bir RPMFusion'lu Fedoradan farkı nedir?

Ultramarine Linux, Fedora'nın yasal sınırlamalarından kaynaklanan hayal kırıklığından doğdu. Fedora (ve Red Hat) Amerikan bir kuruluş olduğu için, ABD patent sistemi nedeniyle dağıtıma hangi yazılımların paketlenebileceği konusunda yasal kısıtlamalar bulunmaktadır.

Öte yandan, Ultramarine Tayland'da bulunmakta ve hangi yazılımların paketlenebileceğine dair herhangi bir telif hakkı kısıtlamasına tabi değildir.

Ultramarine ekibi, kullanıcıların Flathub, RPMFusion veya Terra'da sürdürüldüğü sürece istedikleri herhangi bir yazılımı kurmasına veya paketlemesine izin vererek Fedora'yı biraz daha kullanıcı dostu hale getirmeyi amaçlamaktadır.

Buna ek olarak, sistem etrafında çeşitli kullanıcı deneyimi iyileştirmeleri sağlıyoruz ve gelecekte özel uygulamalar sunmayı planlıyoruz.

Ultramarine, kullanıcıların projeye katkıda bulunmalarını şiddetle teşvik eder, başlamak için [Katkıda Bulunma Rehberi](/tr/contributing/contributorguide)'ni inceleyin!

### Ultramarine sadece Fedora mı, ek masaüstü ortamları ile mi sınırlı?
